...
/**
 * This is a transparent HTTP proxy written in Java that is similar to the proxy in the OpenLayers examples, which is
 * written in Python. These proxies are used to circumvent browser restrictions on cross-domain requests with
 * Javascript. </p>
 * <p>
 * To use the proxy you need to 1) configure the proxy servlet in your web.xml and 2) use OpenLayers.setProxyHost to set
 * the url-path to the proxy. If the proxy is configured to listen to the url-pattern '/gwtOpenLayersProxy/*' then the
 * proxy host should be set to 'gwtOpenLayersProxy?targetURL='.
 * </p>
 * Initial code for this proxy is based upon <a href=
 * "http://trac.openlayers.org/changeset/8099/sandbox?format=diff&new=8099">the following code</a><br />
 * see also <a href= "http://java.sun.com/docs/books/tutorial/networking/urls/readingWriting.html"
 * title="this networking tutorial">this networking tutorial</a>
 * <p>
 */
@SuppressWarnings("serial")
public class OpenLayersProxy extends HttpServlet {
  ...
  private void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException,
    ...
    boolean allowedHost = isAllowedHost(remoteHost); // The allowedHosts are the hosts that are allowed to use the Open
                                                     // Proxy.
    ...
    try {
      ...
      if (request.getParameter("targetURL") != null && request.getParameter("targetURL") != "" && allowedHost) {
        ...
        if (requestMethod.equals("POST")) {
          ...
          int clength = request.getContentLength();// clength is for checking if there is a POST body. Is that
                                                   // sufficient?
  ...
  private void transferHTTPRequestHeaders(HttpURLConnection connection, HttpServletRequest request) {
    ...
    if (request.getHeader("Connection") != null) {
      // TODO HTTP/1.1 proxies MUST parse the Connection header field before a message is forwarded and, for each
