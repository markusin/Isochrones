...
import isochrones.mrnex.algorithm.test.RuntimePercentageTest;
...
public class RuntimeExperimentPercentage {
  ...
  private void logInFiles(File file) throws IOException {
    ...
    for (int i = 0; i < util.getLocations().length; i++) {
      bF.append(util.getLocations()[i]).append(",");
    }
    bF.append("###\n");
  ...
  public static void main(String[] args) throws SQLException, IOException {
    ...
    BenchmarkUtil util = new BenchmarkUtil(args, '=', "IER");
    ...
    while (i < frequency) {
      ...
      if(totalSize==0){
        totalSize = DBUtility.getTotalRows(util.getConfig().getConnection(),util.getConfig().getEdgeTable());
      }
